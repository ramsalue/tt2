...
El sistema de control actúa integrando la lectura del estado físico del mecanismo (sensores) y la ejecución de los algoritmos de movimiento (procesamiento) para enviar señales a los actuadores y estos puedan realizar los movimientos de flexión-extensión.
...
Los sensores ópticos mostraron sensibilidad a las condiciones de iluminación ambiental, la superficie reflectante del aluminio, y requerían una alineación milimétrica difícil de mantener con las vibraciones del movimiento. Los interruptores mecánicos utilizan una conmutación entre 0 y 1 detectable para lograr el frenado de seguridad.

\textbf{Configuración implementada:}
Se instalaron un total de 4 sensores, alimentados con una tensión lógica de 3.3V proveniente de la Raspberry Pi [Fig. \ref{fig:sensores_limit_switches}]:
\begin{itemize}
...
\begin{figure}[h!]
	\centering
	\subcaptionbox{Limit switch para movimiento de extensión.\label{fig:limit_flexion}}
	{\includegraphics [trim = 0 0cm 0 0, clip,width=6cm]{figure/img_componentes/limit_flexion.png}}
	\subcaptionbox{Limit switch para movimiento de abducción.\label{fig:limit_abduccion}}
	{\includegraphics [trim = 0 0cm 0 0, clip,width=6cm]{figure/img_componentes/limit_abduccion.png}}
	\caption{Sensores limit switches posicionados en mecanismo de flexión-extensión y abducción-aducción.\label{fig:sensores_limit_switches}}
\end{figure}
...
\begin{enumerate}
	...
	\item \textbf{Arreglo pull-up:} Integra resistencias de $10\text{k}\Omega$ en paralelo con capacitores que actuan como un filtro de ruido, conectadas a 3.3V en cada línea de señal. Esto asegura que, cuando el interruptor está abierto, el pin de la Raspberry lea un 1 estable, y solo cambie a 0 cuando el interruptor cierre el circuito a tierra.
\end{enumerate}

\subsubsection{Verificación del módulo de acondicionamiento de señales}
...
\end{itemize}
Estos valores buscan que no existan voltajes intermedios que puedan causar indeterminación en el software. 
...
\subsubsection{Arquitectura del Software (app\_fisioterapia.py)}
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item \textit{Stop:} Estado de bloqueo ante activación de límites de software o hardware.
	\end{enumerate}
\end{itemize}
...
En la Fig. \ref{fig:circuito_control_fisico} se observa el flujo de señales correspondientes al sistema de control, donde la Raspberry Pi 4 se desempeña como el cerebro del sistema coordinando el envío y recepción de señales tal como se esperaba en la etapa de diseño de este sistema.
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\linewidth]{figure/img_componentes/circuito_control.png}
    \caption{Esquema de circuito de conexiones para el sistema de control.}
    \label{fig:circuito_control_fisico}
\end{figure}